...
void AUE5TopDownARPGPlayerController::BeginPlay()
{
	...
	for (int i = 0; i <= 100; i++)
	{
		for (int j = 0; j <= 100; j++)
		{
			RayStart = FVector(i * 10, j * 10, 100);
			RayEnd = FVector(i * 10, j * 10, -100);
			bool bObstructed = NavData->Raycast(RayStart, RayEnd, HitLocation, FSharedConstNavQueryFilter()); // true if line from RayStart to RayEnd is obstructed
			UE_LOG(LogUE5TopDownARPG, Log, TEXT("Raycast at %s, %s: %d"), RayStart, RayEnd, (int)bObstructed);
		}
	}
